...
function requestDownload(item) {
    return new Promise((resolve, reject) => {
        let stream = fs.createWriteStream(path.join(__dirname, 'music', item.name + '.mp3'));
...
async function init() {
    ...
    page.on('response', response => {
        ...
        // console.log("TCL: init -> reqUrl", reqUrl)
        if(resourceType.match(/document/)){
            ...
            if(reqUrl.indexOf('discover/playlist') !== -1){
                response.text().then(str => {
                    ...
                    let curList = getCurPlayList($);
                    console.log("TCL: init -> curList", curList)
            ...
            //处理歌曲列表, 循环歌单得到
                ...
                response.text().then(str => {
                    ...
                    console.log("TCL: init -> songList", songList)
...
init().then(d => console.log(d,111111)).catch(e => console.error(e));
